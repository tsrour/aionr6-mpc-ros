...
# path/track
#track_points_filename = 'data/track_rectangle.mat'
...
#     xi_curr = np.vstack(sol['xi'][:,1]) # take state at second time step for now
    xi_curr = np.array(ltvcmpc.unicycle_raw_taylor_order2_next_step(xi_curr, sol['u'][:,1], Ts))
#    xi_curr = np.array(ltvcmpc.unicycle_raw_taylor_order2_next_step(xi_curr, sol['u'][:,0], Ts)) + np.random.normal(0, sigma)

    # Update history
    history_xi[:,i+1] = xi_curr.T
    history_u[:,i] = sol['u'][:,1]
